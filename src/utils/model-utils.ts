...
import type { FieldKind, PrismaField, PrismaModel } from "../types/generator-types";
	...
	for (const field of model.fields) {
		...
		if (["Int", "Float", "Decimal"].includes(field.type)) {
			if (field.documentation) {
				...
				if (minMatch) fieldConstraints.min = parseInt(minMatch[1], 10);
				if (maxMatch) fieldConstraints.max = parseInt(maxMatch[1], 10);
			}
		}
