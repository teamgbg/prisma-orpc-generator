...
exports.PgFunctionStrategy = void 0;
...
class PgFunctionStrategy {
    ...
    async regenerateAppRouter(models) {
        if (this.pgFunctions.length === 0)
            return;
        ...
        const modelImports = models
            .map((m) => {
            const routerName = (0, pluralize_1.default)(m.name.toLowerCase());
        ...
        const modelReExports = models
            .map((m) => `${(0, pluralize_1.default)(m.name.toLowerCase())}Router`)
            .join(", ");
...
exports.PgFunctionStrategy = PgFunctionStrategy;
//# sourceMappingURL=pg-function-strategy.js.map