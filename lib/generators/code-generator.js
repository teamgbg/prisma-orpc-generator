...
class CodeGenerator {
    ...
    async generateBaseRouter(options) {
        ...
        const createRouterFile = this.projectManager.createSourceFile(path_1.default.resolve(this.outputDir, 'routers', 'helpers', 'createRouter.ts'), undefined, { overwrite: true });
...
export interface ShieldContext {
  ...
  prisma: PrismaClient;
        ...
        const shieldIntegration = this.config.generateShield && this.config.shieldPath ? this.generateShieldIntegration() : '';
...
`;
    ...
    resolveShieldModuleSpecifierForHelpers() {
        if (!this.config.shieldPath) {
            return '../shield';
        }
        const shieldPath = this.config.shieldPath;
        const helpersDir = path_1.default.resolve(this.outputDir, 'routers', 'helpers');
        try {
            // Handle absolute paths
            if (path_1.default.isAbsolute(shieldPath)) {
                const relativePath = path_1.default.relative(helpersDir, shieldPath);
                return relativePath.startsWith('.') ? relativePath : `./${relativePath}`;
            }
            // Handle relative paths - resolve from project root (parent of output dir)
            const projectRoot = path_1.default.dirname(this.outputDir);
            const fullShieldPath = path_1.default.resolve(projectRoot, shieldPath);
            const relativePath = path_1.default.relative(helpersDir, fullShieldPath);
            return relativePath.startsWith('.') ? relativePath : `./${relativePath}`;
        }
        catch (error) {
 ...
 `;
    ...
    async generateModelRouter(model, modelOperations) {
        ...
        const modelRouter = this.projectManager.createSourceFile(path_1.default.resolve(this.outputDir, 'routers', 'models', `${modelName}.router.ts`), undefined, { overwrite: true });
    ...
    async generateModelRouterContent(sourceFile, model, modelOperations) {
        ...
        if (this.config.generateInputValidation || this.config.generateOutputValidation) {
            (0, code_generation_utils_1.generateSchemaImports)(sourceFile, modelName, this.config);
        }
        // Generate procedures
...
`);
  ...
  ${procedureName}: publicProcedure${inputPart}
    ...
    async generateAppRouter(models) {
        ...
        const appRouter = this.projectManager.createSourceFile(path_1.default.resolve(this.outputDir, 'routers', 'index.ts'), undefined, { overwrite: true });
...
`;
    ...
    resolveShieldModuleSpecifier() {
        ...
        try {
            ...
            // Handle absolute paths
            if (path_1.default.isAbsolute(shieldPath)) {
                const relativePath = path_1.default.relative(path_1.default.resolve(this.outputDir, 'routers'), shieldPath);
                return relativePath.startsWith('.') ? relativePath : `./${relativePath}`;
            }
            // Handle relative paths
            const possiblePaths = [
                // Path relative to project root
                path_1.default.resolve(path_1.default.dirname(this.outputDir), shieldPath),
                // Path relative to output dir
                path_1.default.resolve(this.outputDir, shieldPath),
                // Path relative to current working directory
                path_1.default.resolve(process.cwd(), shieldPath),
            ];
            // Find first existing path
            for (const possiblePath of possiblePaths) {
                try {
                    const stats = fs_1.default.statSync(possiblePath);
                    if (stats.isFile()) {
                        const relativePath = path_1.default.relative(path_1.default.resolve(this.outputDir, 'routers'), possiblePath);
    ...
    generateShieldedAppRouter(routerEntries, models) {
        const shieldModuleSpecifier = this.resolveShieldModuleSpecifier();
        return `
/**
 * Main application router combining all model routers
...
`;
    ...
    async generateErrorHandlingModule() {
        const errorHandlingPath = path_1.default.resolve(this.outputDir, 'errorHandling.ts');
