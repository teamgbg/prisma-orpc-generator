 ...
 */
function generateSimpleCall(fn) {
 ...
 */
function generateDynamicCall(fn) {
    ...
    lines.push(`      const sql = 'SELECT ${fn.name}(' + sqlParts.join(', ') + ') AS data';`);
    lines.push(`      const rows = await (ctx.prisma.$queryRawUnsafe as Function)(sql, ...sqlValues);`);
    return {
...
function generateFunctionRouterCode(fn) {
    ...
    const imports = fn.isOrgScoped || fn.isUserScoped ? ["protectedProcedure"] : ["publicProcedure"];
