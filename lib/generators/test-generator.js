...
class TestGenerator {
    ...
    async generateUnitTests(models) {
        for (const model of models) {
            await this.generateModelUnitTests(model);
        }
    }
    async generateModelUnitTests(model) {
        const testFile = this.projectManager.createSourceFile(path_1.default.join(this.outputDir, 'tests', 'unit', `${model.name}.test.ts`), undefined, { overwrite: true });
        testFile.addStatements(`
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { ${(0, pluralize_1.default)(model.name.toLowerCase())}Router as modelRouter } from '../../routers/models/${model.name}.router';
...
`);
    ...
    async generateIntegrationTests(models) {
        const integrationTestFile = this.projectManager.createSourceFile(path_1.default.join(this.outputDir, 'tests', 'integration', 'api.test.ts'), undefined, { overwrite: true });
...
`);
    ...
    async generateTestUtils(models) {
    ...
    async generateMockContext(models) {
        ...
        const mockContextFile = this.projectManager.createSourceFile(path_1.default.join(this.outputDir, 'tests', 'utils', 'mock-context.ts'), undefined, { overwrite: true });
...
`);
    ...
    async generateTestServer() {
        const testServerFile = this.projectManager.createSourceFile(path_1.default.join(this.outputDir, 'tests', 'utils', 'test-server.ts'), undefined, { overwrite: true });
 ...
 export default defineConfig({
   ...
   test: {
     ...
     setupFiles: ['tests/setup.ts']
        ...
        await fs_1.promises.writeFile(path_1.default.join(this.outputDir, 'vitest.config.ts'), vitestConfig);
    }
   ...
   "include": [
     ...
     "zod-schemas/**/*.ts"
        ...
        await fs_1.promises.writeFile(path_1.default.join(this.outputDir, 'tsconfig.json'), tsconfig);
    }
    async generateTestTypes() {
        const file = this.projectManager.createSourceFile(path_1.default.join(this.outputDir, 'tests', 'global.d.ts'), undefined, { overwrite: true });
...
`);
    ...
    async generateTestSetup() {
        const setupFile = this.projectManager.createSourceFile(path_1.default.join(this.outputDir, 'tests', 'setup.ts'), undefined, { overwrite: true });
...
`);
    ...
    generateTestData(model, seed = 1, isUpdate = false, skipTimestamps = false) {
        const data = {};
        for (const field of model.fields) {
            if (field.isId && !isUpdate)
                continue;
            if (field.isReadOnly)
                continue;
            if (isUpdate && field.isId)
                continue;
            if (skipTimestamps && (field.name === 'createdAt' || field.name === 'updatedAt'))
                continue;
            switch (field.type) {
                case 'String':
