...
class ProjectManager {
    ...
    async cleanOutputDirectory() {
        try {
            ...
            const entries = await node_fs_1.promises.readdir(this.outputDir, { withFileTypes: true });
            for (const entry of entries) {
                const fullPath = node_path_1.default.join(this.outputDir, entry.name);
    ...
    async removeDirectory(dirPath) {
        try {
            await node_fs_1.promises.rm(dirPath, { recursive: true, force: true });
        }
        catch {
    ...
    async createDirectoryStructure(directories) {
        for (const dir of directories) {
            await node_fs_1.promises.mkdir(node_path_1.default.join(this.outputDir, dir), { recursive: true });
        }
    }
    ...
    async saveProject() {
        ...
        for (const sf of this.project.getSourceFiles()) {
            ...
            const rel = node_path_1.default.relative(this.outputDir, sf.getFilePath());
    ...
    async generateBarrelExport(directory) {
        const dirPath = node_path_1.default.join(this.outputDir, directory);
        try {
            ...
            for (const entry of entries) {
                if (entry.isFile() && entry.name.endsWith(".ts") && entry.name !== "index.ts") {
                    ...
                    const subIndexPath = node_path_1.default.join(dirPath, entry.name, "index.ts");
                    try {
                        await node_fs_1.promises.access(subIndexPath);
            ...
            if (exports.length > 0) {
                const indexFile = this.createSourceFile(node_path_1.default.join(dirPath, "index.ts"), exports.join("\n"), { overwrite: true });
    ...
    async generatePackageInfo(_config) {
        ...
        await node_fs_1.promises.writeFile(node_path_1.default.join(this.outputDir, "package.json"), JSON.stringify(cleanPackageJson, null, 2));
    }
    getProject() {
        return this.project;
    }
